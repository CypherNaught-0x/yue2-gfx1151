...
class YuE2Pipeline:
    ...
    def synthesize(self, semantic, *, cancelled=None):
        ...
        self.synthesis_timing = {"model_prepare_seconds": time.perf_counter() - prepare_start}
        ...
        self.synthesis_timing["compute_and_host_transfer_seconds"] = time.perf_counter() - compute_start
        self.synthesis_timing["total_seconds"] = time.perf_counter() - total_start
        return array
    ...
    def decode(self, latents, *, full=False, vae=None):
        ...
        self.decode_timing = {"model_load_and_device_transfer_seconds": time.perf_counter() - load_start}
        ...
        self.decode_timing["input_prepare_seconds"] = time.perf_counter() - input_start
        try:
            ...
            with self._status("Decoding audio", total=tiles, unit="chunks") as status:
                ...
                self.decode_timing["decoder_compute_and_host_transfer_seconds"] = time.perf_counter() - decode_start
                if not torch.isfinite(audio).all():
                    raise ValueError("VAE produced non-finite audio")
                post_start = time.perf_counter()
                output = audio[0].float().clamp(-1, 1).T.contiguous().numpy()
                self.decode_timing["output_postprocess_seconds"] = time.perf_counter() - post_start
                return output
        finally:
            ...
            if self.device.type == "cuda":
                torch.cuda.empty_cache()
            self.decode_timing["cleanup_seconds"] = time.perf_counter() - cleanup_start
            self.decode_timing["total_seconds"] = time.perf_counter() - total_start
    ...
    def __call__(self, style=None, lyrics=None, *, tags=None, abc_sampling=None,
        ...
        timing = {"abc": plan.timing, "semantic": semantic.timing, "nar_seconds": nar_seconds,
                  "nar": dict(self.synthesis_timing),
                  "vae_seconds": time.perf_counter() - vae_start, "vae": dict(self.decode_timing),
                  "load": dict(self.load_timing),
