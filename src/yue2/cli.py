...
def generate(args):
    ...
    pipe = get_pipe(args)
    pipe_ready_seconds = time.perf_counter() - command_start
    ...
    try:
        if args.stage == "plan":
            ...
            result.timing["cli"] = {
                "pipe_setup_seconds": pipe_ready_seconds,
                "generation_call_seconds": generation_seconds,
                "artifact_save_seconds": artifact_save_seconds,
                "total_through_save_seconds": time.perf_counter() - command_start,
            }
            receipt["timing"] = result.timing
