...
class SnakeBeta(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.alpha_logscale:
            ...
            beta = torch.exp(beta)
        fused = getattr(self, "_fused_snake_impl", None)
        if (fused is not None and not self.training
                and not torch.is_grad_enabled() and x.device.type == "cuda"
                and x.dtype == torch.float32):
            return fused(x, alpha, beta)
